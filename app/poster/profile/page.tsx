...
const profileSchema = z.object({
  ...
  location: z.string().nullable().or(z.string().length(0)),
...
export default function PosterProfilePage() {
  ...
  // Handle image conversion to base64 and upload
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    // Check size limit (e.g. 4MB max)
    if (file.size > 4 * 1024 * 1024) {
      setErrorMsg('Image size too large. Maximum size is 4MB.');
      return;
    }
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
      ...
      <div>
        ...
        <p className="text-slate-500 mt-1">Configure your personal information, avatar, and review job history statistics</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
            <h4 className="font-bold text-sm text-primary uppercase tracking-wider">Posted Task Summary</h4>
            <div className="space-y-3">
              <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
                <span className="text-slate-500 flex items-center gap-1.5"><Briefcase size={16} /> Total Posted</span>
                <span className="font-bold text-primary">{stats.totalPosted}</span>
              </div>
              <div className="flex justify-between items-center text-sm border-b border-slate-100 pb-2">
              ...
              <div className="flex justify-between items-center text-sm">
                <span className="text-slate-500 flex items-center gap-1.5"><Clock size={16} className="text-blue-500" /> Active / Pending</span>
                <span className="font-bold text-primary">{stats.inProgress + stats.open}</span>
              </div>
            </div>
          </div>
        </div>
