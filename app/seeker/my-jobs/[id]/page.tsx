...
export default function SeekerJobDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const fetchJobDetails = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        throw new Error('Failed to load job details. You may not have permission.');
      }
  ...
  const [posterCoords, setPosterCoords] = useState<{ latitude: number; longitude: number; recordedAt: string } | null>(null);
  const [loadingCoords, setLoadingCoords] = useState(false);
  const [coordsError, setCoordsError] = useState<string | null>(null);

  const fetchPosterGPS = async (posterId: string) => {
    setLoadingCoords(true);
    setCoordsError(null);
    try {
      const res = await fetch(`/api/gps?userId=${encodeURIComponent(posterId)}`);
      if (!res.ok) {
        if (res.status === 404) {
          throw new Error('Client has not shared any location logs yet.');
        }
        throw new Error('Failed to retrieve client coordinates.');
      }
      const data = await res.json();
      if (data.success && data.log) {
        setPosterCoords(data.log);
      }
    } catch (err: any) {
  ...
  const isLocationLive = () => {
    if (!posterCoords) return false;
    const diff = Date.now() - new Date(posterCoords.recordedAt).getTime();
    return diff < 120000;
  };
  ...
  if (error || !task) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center space-y-4">
        ...
        <h2 className="text-xl font-bold text-primary">Error Loading Job</h2>
        <p className="text-slate-500">{error || 'Job details could not be found.'}</p>
        <Link
          href="/seeker/my-jobs"
          className="inline-flex items-center gap-1.5 text-sm font-bold text-accent hover:underline"
        >
          <ArrowLeft size={16} /> Back to My Active Jobs
        </Link>
      </div>
    );
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-100 shadow-sm space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-start gap-2.5 text-sm">
                  ...
                  <div>
                    ...
                    <div className="text-primary font-bold text-lg">{task.budget.toLocaleString()} BDT</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="p-4 rounded-xl bg-slate-50 border border-slate-100 space-y-2 mt-4">
              <h4 className="font-bold text-xs text-slate-400 uppercase tracking-wider">Posted By</h4>
              <div className="text-sm">
                <div className="font-bold text-primary">{task.posterName || 'Verified Client'}</div>
                <div className="text-xs text-slate-500">{task.posterPhone}</div>
              </div>
            </div>
          </div>

          {/* Client Location Tracking Panel */}
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div>
                <h3 className="font-bold text-primary text-base flex items-center gap-2">
                  <Navigation size={18} className="text-accent" /> Client Location
                </h3>
                <p className="text-xs text-slate-400 mt-0.5">
                  {posterCoords
                    ? isLocationLive()
                      ...
                      : 'Last known location'
                    : 'Awaiting client GPS signal'}
                </p>
              </div>
              {posterCoords && (
                <span className={`flex items-center gap-1.5 text-xs font-bold px-2 py-1 rounded-full ${isLocationLive()
                    ? 'bg-emerald-50 text-emerald-700'
                    : 'bg-slate-100 text-slate-500'
                  }`}>
                  <span className={`w-2 h-2 rounded-full ${isLocationLive() ? 'bg-emerald-500 animate-pulse' : 'bg-slate-400'}`} />
                  {isLocationLive() ? 'Live' : 'Offline'}
                </span>
              )}
            </div>

            {loadingCoords && !posterCoords && (
            ...
            {!loadingCoords && coordsError && !posterCoords && (
              <div className="flex flex-col items-center gap-2 p-6 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-center">
                ...
                <button
                  onClick={() => task && fetchPosterGPS(task.posterId)}
            ...
            {posterCoords && (
              <div className="space-y-3">
                <div className="h-[280px] relative overflow-hidden rounded-xl border border-slate-100">
                  <TrackingMap
                    latitude={posterCoords.latitude}
                    longitude={posterCoords.longitude}
                    seekerName={task?.posterName || 'Client'}
                  />
                </div>
                <div className="text-[11px] text-slate-400 text-right">
                  Last updated: {new Date(posterCoords.recordedAt).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            )}
          </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm space-y-6">
            <div>
              <h3 className="font-bold text-primary text-base">Job Tracking Status</h3>
            ...
            {task.status === 'CANCELLED' ? (
              ...
              <div className="pt-2">
                <ProgressBar currentStep={task.progressStep} role="SEEKER" isVertical={true} />
              </div>
            )}
          </div>
